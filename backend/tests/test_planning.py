...
from jobs.models import Question, Scene
from jobs.tasks import plan_ad

from .conftest import PLAN, READABLE, openai_reply
...
def test_a_plan_that_breaks_the_rules_fails_the_job_and_nothing_of_it_is_kept(
    ...
    openai_server(
        ...
        openai_reply({"type": "output_text", "text": json.dumps(reply), "annotations": []}),
    )
...
def test_the_producers_question_waits_for_an_answer_and_the_plan_uses_it(
    ...
    assert waiting["question"] == {"kind": "producer", "question": ASK["question"]}
...
def test_a_restart_while_waiting_for_an_answer_keeps_waiting_without_asking_again(
    ...
    assert after["question"] == {"kind": "producer", "question": ASK["question"]}
